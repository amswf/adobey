...
package fl.video {
	...
	public class UIManager {
		...
		public function UIManager(vc:FLVPlayback) {
			...
			_fullScreen = false;
			// work around for iPhone problems. On iPhone, we START in fullscreen mode, which causes all
			// sorts of nightmares for FLVPlayback because it tries to go into its fullscree take over
			// mode as soon as it is created and placed on stage, which was never intended and breaks
			// things badly. To avoid causing backward compatibility problems, making smallest fix possible,
			// which is to default fullScreenTakeOver to false on iPhone only.
			_fullScreenTakeOver = (Capabilities.os.indexOf("iPhone") < 0);
			_fullScreenBgColor = 0x000000;
			_fullScreenAccel = false;
			if (_vc.stage != null) {
				_vc.stage.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE, handleMouseFocusChangeEvent, false, 0, true);
				try {
					CONFIG::PLAYER {
					...
					CONFIG::AIR {
						_fullScreen
							...
							||	_vc.stage.displayState == StageDisplayState.FULL_SCREEN_INTERACTIVE
							;						
					}
					_vc.stage.addEventListener(FullScreenEvent.FULL_SCREEN, handleFullScreenEvent, false, 0, true);
		...
		flvplayback_internal function handleLayoutEvent(e:LayoutEvent):void {
			if (_fullScreen && _fullScreenTakeOver && _fullScreenAccel && _vc.stage != null) {
				...
				if ( _vc.registrationX != 0 ||_vc.registrationY != 0 ||
				     ...
				     ( _fullScreenAccel &&
				       (_vc.registrationWidth != _vc.stage.fullScreenSourceRect.width || _vc.registrationHeight != _vc.stage.fullScreenSourceRect.height) ) ||
				     ( !_fullScreenAccel &&
				       (_vc.registrationWidth != _vc.stage.stageWidth || _vc.registrationHeight != _vc.stage.stageHeight) ) )
				{
		...
		flvplayback_internal function handleButtonEvent(e:MouseEvent):void {
			...
			case MouseEvent.MOUSE_DOWN:
				...
				topLevel.addEventListener(MouseEvent.MOUSE_OUT, captureMouseEvent, true, 0, true);
				topLevel.addEventListener(MouseEvent.MOUSE_OVER, captureMouseEvent, true, 0, true);
				topLevel.addEventListener(MouseEvent.MOUSE_UP, handleMouseUp, false, 0, true);
				topLevel.addEventListener(MouseEvent.ROLL_OUT, captureMouseEvent, true, 0, true);
				topLevel.addEventListener(MouseEvent.ROLL_OVER, captureMouseEvent, true, 0, true);
				break;
			...
			if (!_fullScreen && _vc.stage != null) {
				if (_fullScreenTakeOver) {
					try {
						...
						CONFIG::AIR {
							// Can be either interactive, or not. In AIR, we'll use the interactive mode:
							_vc.stage.displayState = StageDisplayState.FULL_SCREEN_INTERACTIVE;
						}
						CONFIG::PLAYER {
							_vc.stage.displayState = StageDisplayState.FULL_SCREEN;
		...
		flvplayback_internal function enterFullScreenTakeOver():void {
			...
			try {
				...
				_vc.registrationY = 0;
				if (_fullScreenAccel) {
					_vc.setSize(_vc.stage.fullScreenSourceRect.width, _vc.stage.fullScreenSourceRect.height);
				} else {
				...
				if (_fullScreenAccel) {
					fullScreenBG.graphics.drawRect(0, 0, _vc.stage.fullScreenSourceRect.width, _vc.stage.fullScreenSourceRect.height);
				} else {
				...
				if(hitTarget_mc != null){
					...
					if (_fullScreenAccel) {
						hitTarget_mc.graphics.drawRect(0,0,_vc.stage.fullScreenSourceRect.width, _vc.stage.fullScreenSourceRect.height);
					} else {
