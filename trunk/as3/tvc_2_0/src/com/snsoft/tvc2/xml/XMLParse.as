package com.snsoft.tvc2.xml {
	...
	public class XMLParse {
		...
		public static const TAG_MAP_LISTE:String = "mapList";

		public static const TAG_MAP_DES:String = "mapDes";
		...
		public static const ATT_FILE:String = "file";
		...
		public function parseMapsDes(xml:XML):MapsDesesDO {
			var mapsDO:MapsDesesDO = new MapsDesesDO();
			var xc:XMLDom = new XMLDom(xml);
			var node:Node = xc.parse();
			var nodeList:NodeList = node.getNodeList(TAG_MAP_LISTE);
			for (var i:int = 0; i < nodeList.length(); i++) {
				var cnode:Node = nodeList.getNode(i);
				var cnodeList:NodeList = cnode.getNodeList(TAG_MAP_DES);
				for (var j:int = 0; j < cnodeList.length(); j++) {
					var ccnode:Node = cnodeList.getNode(j);
					var mddo:MapDesDO = new MapDesDO();
					mddo.name = ccnode.getAttributeByName(ATT_NAME);
					mddo.value = ccnode.getAttributeByName(ATT_VALUE);
					mddo.text = ccnode.getAttributeByName(ATT_TEXT);
					mddo.file = ccnode.getAttributeByName(ATT_FILE);
					mddo.x = ccnode.getAttributeByName(ATT_X);
					mddo.y = ccnode.getAttributeByName(ATT_Y);
					mddo.z = ccnode.getAttributeByName(ATT_Z);
					mddo.s = ccnode.getAttributeByName(ATT_S);
					mapsDO.addMapDesDO(mddo);
				}
			}
			return mapsDO;
		}
