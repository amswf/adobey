		...
		private function play():void {
			if (!isPlay) {
				...
				if (bizDO != null) {
					...
					dataDO.bizSoundList = new Vector.<Vector.<Sound>>();

					var resSetList:Vector.<RSSound> = dataDO.resSetList;
					if (resSetList != null) {
						for (var ri:int = 0; ri < resSetList.length; ri++) {
							var rssound:RSSound = resSetList[ri];
							dataDO.bizSoundList.push(rssound.soundList);
						}
					}
