		...
		public function format(nNumber:Number, oParameter1:Object = null, oParameter2:Object = null):String {
			...
			if(_sMask == null) {
				...
				for(var ii:Number = 0; ii < aMask.length; ii++) {
					if(aMask[ii] != "0" && aMask[ii] != "#" && aMask[ii] != ".") {
						aMask.splice(ii, 1);
						ii--;
					}
				}
				...
				if(nCounter < aPart0.length) {
					for(var i3:Number = 0; i3 < aPart0.length - nCounter; i3++) {
						sNumber += aPart0[i3];
						nPart0Index++;
					}
				}
					...
					for(var i4:Number = 0; i4 < nCounter - aPart0.length; i4++) {
						if(aMask0[i4] == "0") {
							sNumber += "0";
						}
						else if (aMask0[i4] == "#") {
				...
				for(var i5:Number = nMaskIndex; i5 < aMask0.length; i5++) {
				...
				for(var i7:Number = 0; i7 < aMask1.length; i7++) {
					...
					if(aPart1[i7] == null) {
						...
						nDigits = Number(aPart1[i7] + "" + aPart1[i7 + 1]);
						if(i7 == aMask1.length - 1 && !isNaN(nDigits)) {
