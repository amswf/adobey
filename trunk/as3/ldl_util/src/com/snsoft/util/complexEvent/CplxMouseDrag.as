	...
	public class CplxMouseDrag extends EventDispatcher
	{
		...
		private function setViewPlace(d:DisplayObject,l:DisplayObject,vd:DisplayObject,vl:DisplayObject):void{
			...
			if(this.dragRect != null){
				...
				dlp = PointUtil.subPoint(dragSize,lSize);
			...
			if(this.viewDragRect != null){
				...
				dlsp = PointUtil.subPoint(viewDragSize,vlSize);
		...
		private function handlerCnMouseMove(e:Event):void{
			...
			if(poc != null){
				if(this.cuntryNameMoveSign){
					...
					PointUtil.setSpritePlace(doc,docp);
		...
		private function calculateMoveLimitPoint(dragDisplayObjectPlace:Point):Point{
			...
			if(ldo != null){
				//宽高
				var dsp:Point = PointUtil.getSpriteSize(ddo);//拖动对象
				var lsp:Point = PointUtil.getSpriteSize(ldo);//限制对象
				...
				//宽高差
				var ssp:Point = PointUtil.subPoint(dsp,lsp);
				
				//位置
				var dpp:Point = dragDisplayObjectPlace;//拖动对象
				var lpp:Point = PointUtil.getSpritePlace(ldo);//限制对象
				
				if(this.dragRect != null){
					var dragPlace:Point = RectangleUtil.getPlace(this.dragRect);
					lpp = PointUtil.subPoint(lpp,dragPlace);
				}
				
				//位置差
				var spp:Point = PointUtil.subPoint(dpp,lpp);
