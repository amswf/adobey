package com.snsoft.core {
	...
	public class UILunarCalendar extends UIComponent {
		...
		private function refreshCalendarDayTimeViewTodayDate(tmc:MovieClip):void {
			if (tmc != null) {
		...
		private function createCalendarDayMore(dayMc:MovieClip, dl:DateLunar):MovieClip {
			...
			if (cdm != null && dayMc != null && dl != null) {
				...
				if (topView) {
					var shape1:Shape = ShapeUtil.drawShapeWithPoint(color, alpha, new Point(p1.x, p1.y), new Point(p2.x, p2.y), new Point(dayMcPoint.x, dayMcPoint.y));
					main.addChild(shape1);
				}
				if (rightView) {
					var shape2:Shape = ShapeUtil.drawShapeWithPoint(color, alpha, new Point(p2.x, p2.y), new Point(p3.x, p3.y), new Point(dayMcPoint.x, dayMcPoint.y));
					main.addChild(shape2);
				}
				if (bottomView) {
					var shape3:Shape = ShapeUtil.drawShapeWithPoint(color, alpha, new Point(p3.x, p3.y), new Point(p4.x, p4.y), new Point(dayMcPoint.x, dayMcPoint.y));
					main.addChild(shape3);
				}
				if (leftView) {
					var shape4:Shape = ShapeUtil.drawShapeWithPoint(color, alpha, new Point(p4.x, p4.y), new Point(p1.x, p1.y), new Point(dayMcPoint.x, dayMcPoint.y));
