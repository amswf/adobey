package tvc {
	...
	public class AlterableCoordinates {
		...
		public function getNewMaxY(min:Number, max:Number, ygn:Number):Number {
			var graduationY:Number = (this.max_y - this.min_y) / ygn;
			var power:Number = 1;
			while (graduationY < 1) {
				graduationY*=10;
				power*=10;
		...
		public function dodraw(shape:Shape, moviec:MovieClip):void {
			...
			var gridValue_x:Number = (this.max_x - this.min_x) / this.X_GraduationNum;
			...
			var gridValue_y:Number = (this.max_y - this.min_y) / this.Y_GraduationNum;
		...
		public function drawXGradTextOfLine(movie:MovieClip):void {
			...
			for (var j:int = 0; j < textXArray.length; j++) {
				...
				var reda:Array=this.gettrueValueRedPointAry();
				pc=reda[j];
